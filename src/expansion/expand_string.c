...
/*   Updated: 2025/12/14 11:20:33 by ssukhija         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static char	*process_squote(char *str, int *i, char *res)
{
...
static char	*expand_quotes(char *str, int *i, char *res, int *in_dq)
{
	...
	return (process_squote(str, i, res));
}
...
char	*expand_string(char *str, t_shell *shell)
{
	...
	while (str[i])
	{
		if (str[i] == S_QM_S || str[i] == D_QM_S || str[i] == D_QM_E)
			res = expand_quotes(str, &i, res, &in_dquote);
		else if (str[i] == '\\')
